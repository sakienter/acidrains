...
function castHeadhunter(gameState) {
  discoverCards(
    gameState,
    MINIONS.filter(card => typeof card.battlecry === "function"),
    1,
    "ヘッドハンター：雄叫びミニオンを発見"
  );
}
...
function castRememberTheBeginning(gameState) {
  const gifts = [TOKEN_CARDS.gift].filter(Boolean);
  if (!gifts.length) {
...
function castScroll(gameState) {
  ...
  discoverCards(
    gameState,
    SPELLS.filter(card => Number(card.tier) === tier && card.id !== "scroll"),
    1,
...
window.addEventListener("load", () => {
  ...
  updateAuras = function() {
    ...
    if (state.brannSpellActive) {
      state.battlecryMultiplier = Math.max(2, Number(state.battlecryMultiplier || 1));
    }
  };
