/* Fixed, name-specific card icons. */
(() => {
  const ICONS = {
    'コイン': '🪙',
    '新芽': '🌱',
  ...
  function applyNamedIcons() {
    const apply = card => { if (card && ICONS[card.name]) card.emoji = ICONS[card.name]; };
  ...
  const timer = setInterval(() => {
    ...
    if (tries >= 80 || (typeof MINIONS !== "undefined" && MINIONS.some(card => ICONS[card.name]))) clearInterval(timer);
  }, 50);
